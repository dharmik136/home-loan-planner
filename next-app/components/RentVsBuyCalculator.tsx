...
export function RentVsBuyCalculator() {
  ...
  for (let y = 1; y <= MAX_YEARS; y++) {
    ...
    // Net buy cost = cash outflows less the terminal asset value.
    const netBuyCost = cumulativeBuyCost - propertyValue;
    ...
    <div className="panel" style={{ marginTop: "16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "12px" }}>
        ...
        <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "3px", padding: "10px", textAlign: "center" }}>
          ...
          <div style={{ fontWeight: "800", fontSize: "1rem", color: finalBuy < finalRent ? "var(--emerald)" : "var(--clay)" }}>{formatINR(finalBuy)}</div>
        </div>
      </div>
      ...
      <div style={{ fontSize: "0.68rem", color: "var(--ink-faint)", marginTop: "8px", lineHeight: "1.3" }}>
        Net buy cost equals the down payment, EMIs, and maintenance less the estimated terminal property value. This does not include tax benefits, opportunity cost of down payment, selling costs, or rental yield.
      </div>
    </div>
  );
}
