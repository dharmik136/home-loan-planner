...
export function PortfolioBalanceChart({ results }: Props) {
  ...
  const colors = ["#2F6B4A", "#B98A2E", "#6E5A4D", "#A8603D", "#8B5A5A", "#8A7A67"];
...
function PortfolioTip({ active, payload, label, results, colors }: any) {
  if (!active || !payload?.length) return null;

  const totalBaseline = payload.find((p: any) => p.dataKey === "totalBaseline")?.value ?? 0;
  
  // Calculate total plan balance this month
  let totalPlan = 0;
  const loanPlanBalances = results.map((r: any, idx: number) => {
    const val = payload.find((p: any) => p.dataKey === `plan_${r.loan.id}`)?.value ?? 0;
