...
export function computeRollover(
  ...
  let totalPaid = 0;
  let m = 0;
  let carriedEmiBudget = 0;

  const markPaidOff = (state: LoanState) => {
    state.balance = 0;
    if (payoffMonths[state.loan.id] === undefined) {
      payoffMonths[state.loan.id] = m;
      // Reuse this loan's EMI from the following month onward.
      carriedEmiBudget += state.baseEmi;
    }
  };
  ...
  while (states.some((s) => s.balance > 0.005) && m < maxSimMonths) {
    ...
    let totalTargetPayment = extraBudget + carriedEmiBudget;
